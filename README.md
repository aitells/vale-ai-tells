...
This package contains 112 rule files covering different categories of AI tells. All rules default to `error` level.
...
| `FigurativeExercises` | Every verb form of "exercise": a quickstart that "exercises" services, a test that "exercises" a code path, a reviewer who "exercises caution," or a caller who "exercises its right." A part-of-speech gate keeps the noun forms quiet (workouts and textbook exercises). The standard technical testing sense fires by design because it names coverage without saying whether the test starts, calls, checks, or verifies the component; disable the rule for legal or fitness prose. |
