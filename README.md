...
Packages = https://github.com/tbhb/vale-ai-tells/releases/download/v1.32.0/ai-tells.zip, \
  https://github.com/tbhb/vale-ai-tells/releases/download/v1.32.0/ai-tells-commits.zip
...
This package contains 128 rule files covering different categories of AI tells. All rules default to `error` level.
...
| `FigurativeNouns` | Nominalized figuration, which every verb-anchored rule misses: "reach" as a retrieval path ("its primary reach," "per-rule reach," "picks between two reaches"), "wrinkle" as a complication, "story" as a design ("the migration story," "a similar story applies"), and "knob" as an option. Each gates on the determiner or compound that makes the noun figurative, at zero to two pre-LLM corpus hits apiece, and the plural "reaches" is admitted only at a clause end or before a copula so "the count reaches 0" stays with the verb. "Heartbeat," "waterfall," "gap," and "the poles" stay out as terms of art or too ordinary to gate. Disable the rule for anatomy, textiles, radio, or fiction writing. |
