...
- **FigurativeClears** (`ai-tells`): New rule. Reports passing dressed as a jump: a draft that "clears the gate," a message that "cleared every check," a fix "clearing a finding," a branch that "clears CI." The checking noun after the verb is the gate, so clearing a cache or a screen stays quiet.
